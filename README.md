...
Baixa o documento **selecionado** na árvore — **sem** a janela nativa "Salvar
como" nem a pasta de download do Chrome. `DownloadDocumento` extrai a URL de
download e busca o arquivo com `fetch()` **dentro da sessão logada** (reusa
cookies e SSL, o que ainda resolve os certificados de sites `.gov.br`), e devolve
o conteúdo como **dado**:
...
DocumentosArvore(driver).selecionar("35551895")    # aponta o documento externo

try:
    doc = DownloadDocumento(driver).baixar()        # DocumentoBaixado: bytes + metadados
except DownloadDocumentoError as exc:
    ...  # URL não encontrada, fetch falhou, ou conteúdo ilegível

print(len(doc.conteudo), doc.content_type, doc.extensao, doc.nome_sugerido)
doc.salvar("downloads")                             # grava downloads/<nome>.<ext>
```

A biblioteca **não escreve em disco por si**: `baixar()` devolve os bytes e os
metadados (`content_type`, `extensao`, `nome_sugerido` — este vindo do
`Content-Disposition`); chame `salvar(pasta, nome=...)` quando quiser gravar.
Vale para documentos **externos/enviados** (PDF, DOCX…); um documento **interno**
do SEI é HTML gerado pelo sistema e não tem anexo para baixar.
...
não um relatório: os campos saem da tela. Além dos dados da pessoa, o
resultado diz se há procuração cadastrada. Este módulo **não produz
documento** — o arquivo que o próprio CIS gera para essa transação não é
capturável pela automação, e a alternativa (imprimir a tela) só traz a
região visível, insuficiente para um cadastro; ver `docs/uso-basico.md`:
...
print(dados.municipio, dados.com_procuracao)
```
...
| `integra_gov.esiape.impressao` | duas mecânicas de impressão: via popup (Save as PDF), a sequência estabilizada — só serve telas de RELATÓRIO, que produzem o PDF por download; e via DevTools (`imprimir_pagina_para_pdf`), que imprime a página atual sem popup nem download — só cobre a região visível, incompleta numa tela cujo conteúdo relevante rola dentro de um frame | ✅ |
...
| `integra_gov.esiape.dados_pensionista` | `CDCOPSBENE`: lê os 12 campos cadastrais do pensionista (os 11 do formulário mais a matrícula pedida) e cruza a tela de procuração; não produz documento (o PDF do CIS não é capturável, e a impressão da tela só traz a região visível) | ✅ |
